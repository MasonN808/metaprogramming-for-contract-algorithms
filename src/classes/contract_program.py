...
class ContractProgram:
        ...
        for time_allocation in refactored_allocations:
            ...
            # node.expression_type == "conditional" and node.in_subtree is False
                ...
                if original_allocations_conditional_branches:
                    ...
                    copied_branch_allocations = [copy.deepcopy(node.true_subprogram.allocations), copy.deepcopy(node.false_subprogram.allocations)]
    ...
    def naive_hill_climbing_outer(self, decay=1.1, threshold=.01, verbose=False, inner=False) -> [float]:
        ...
        while time_switched > threshold:
            ...
            for permutation in permutations(refactored_allocations, 2):
                ...
                # Avoids negative time allocation
                    ...
                    if self.global_expected_utility(adjusted_allocations) > self.global_expected_utility(self.allocations, original_allocations_conditional_branches=self.original_allocations_conditional_branches):
                    ...
                    eu_original = self.global_expected_utility(
                        self.allocations, original_allocations_conditional_branches=self.original_allocations_conditional_branches) * self.scale
        ...
        return [self.allocations, self.original_allocations_conditional_branches[0], self.original_allocations_conditional_branches[1]]
        ...
        if taxed_budget > 0:
            ...
            for time_allocation in self.allocations:
                if time_allocation.time is not None and not Node.is_conditional_node(
                        self.find_node(time_allocation.node_id, self.program_dag)):
