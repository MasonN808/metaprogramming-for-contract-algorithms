...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------
    # Leaf node
    node_2 = Node(2, [], [], expression_type="contract")
    ...
    for _ in tqdm(range(0, SIMULATIONS), desc='Progress Bar', position=0, leave=True):
        ...
        test.save_eu_time_data(eu_time_list=eu_time, eu_file_path="src/tests/small-func/data/eu-data.txt", time_file_path="src/tests/small-func/data/time-data.txt", node_indicies=node_indicies_list)
